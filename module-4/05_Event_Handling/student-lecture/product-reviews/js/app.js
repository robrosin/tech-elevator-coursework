...
function initializePage() {
    ...
    displayReviews();
    // Handle click on description and show input box while hiding description
    // 1. target description
    let desc = document.querySelector('.description');
    // 2. call toggleDescriptionEdit function
    desc.addEventListener('click', (ev) => {
        toggleDescriptionEdit(desc);
    });

    // Handle keyup on description edit
    let desc2 = document.getElementById('inputDesc');
    desc2.addEventListener('keyup', (ev) => {
        if (ev.key === 'Enter') {
