...
namespace WorldDB.DAL
{
    public class CountrySqlDAO : ICountryDAO
    {
        ...
        public IList<Country> SearchCountriesByNameAndContinent(string nameLike, string continentLike)
        {
            // Declare the output variable
            List<Country> countries = new List<Country>();
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    // Don't do this!  We'll see why tomorrow!
                    // string sql = $"Select * from country where name like '%{nameLike}%' and continent like '%{continentLike}%';";
